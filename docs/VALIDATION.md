...
4. **Let the tail record.** Keep recording for at least 3 seconds after the last
   note. Space and Echo are measured from decay tails; a take that stops dead has
...
the score, but clipping the pedal's input is not — it manufactures Grit that
isn't in the preset.

### If you'd rather play it than reamp

Also fine, and better for judging feel. Record yourself playing the same part
through the preset. Be aware that a different performance moves **Squash** and
**Grit** on its own — that's playing dynamics, not the preset. Reamping the app's
...
| # | Test | Do this | Pass |
|---|---|---|---|
| **T1** | Null loop | Bypass every block on the MG (or set the preset to a flat clean with no FX), reamp `tone_N.wav` through it, validate against **itself** rather than the song | Grit/Body/Bite/Honk within tolerance. Anything else means the interface, level or routing is colouring the signal, and every later result is suspect |
...
|---|---|---|---|---|
| **T4** | AMP Gain +25 | Grit up | Body, Bite, Honk within tolerance | Grit rises ≥10 |
| **T5** | AMP Bass +25 | Body up | Grit, Bite | Body rises ≥10 |
| **T6** | AMP Treble +25 | Bite up | Body, Grit | Bite rises ≥10 |
| **T7** | AMP Middle +25 | Honk up | Body, Bite | Honk rises ≥10 |
| **T8** | COMP on, Sustain 80 | Squash up | Body, Bite, Honk | Squash rises ≥12 |
| **T9** | RVB Level 0 → 70 | Space up | everything else | Space rises ≥20 |
| **T10** | DLY E.Level 0 → 70 | Echo up | Body, Bite | Echo rises ≥20 |
| **T11** | MOD Depth 0 → 80 | Swirl up | Grit, Body | Swirl rises ≥20 |
| **T12** | EFX Touch Wah on | Wah/Bite move | Body | Something moves — Wah is a taste axis, not a measured one, so this is a smoke test only |
...
`overall_score` weights Grit, Body, Bite and Honk at 1.6× because they are the
...
weaker (Swirl 20, Wah 25, Grit only 8).
